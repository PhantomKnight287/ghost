...
/**
 * The signature of every named commit, keyed by sha. Commits that carry none -
 * and signatures in a format other than OpenPGP, such as the SSH ones git can
 * also write - are simply absent from the map.
 *
 * One `cat-file --batch` answers a whole page: `git log` cannot hand back the
 * signature header, and a spawn per commit would be a spawn per row.
 */
export async function readSignedCommits({
  ...
  // `<sha> <type> <size>\n<size bytes>\n`, repeated. The size is the only safe
  // boundary: a commit message can hold anything, newlines included.
  let offset = 0;
 ...
 *
 * git signs the commit object as it would have been written without the
 * `gpgsig` header, so verification has to remove exactly that header and its
 * continuation lines - every other byte, header order included, is part of the
 * payload.
 */
export function splitSignature(commit: string): SignedCommit | null {
  ...
  for (const [index, line] of lines.entries()) {
    if (inSignature) {
      // Headers end at the blank line before the message; a continuation line
      // of the signature is the only other line that starts with a space.
      if (line.startsWith(' ')) {
    ...
    // Only a header can carry the signature, and headers stop at the first
    // blank line - a message body quoting `gpgsig ` must not be mistaken for it.
    if (
      signature.length === 0 &&
