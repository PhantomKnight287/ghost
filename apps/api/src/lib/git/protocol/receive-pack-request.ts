...
/** Git cannot rewind a chunked request to retry with credentials, so a large push is preceded by a throwaway POST of one flush packet. Answering it with an error kills the push before the body is sent. */
...
/** Reads only the command section, leaving the packfile on disk. A 2 GiB push cannot be a Buffer - `Hash.update` and most Buffer operations cap at INT_MAX. */
