...
describe('listTree', () => {
  ...
  /** Git reads a leading ":" as pathspec magic, and ":/" means "from the root". Without :(literal) this listing would quietly return the root instead of the directory that was asked for. */
...
describe('orderTreeEntries', () => {
  ...
  it('puts directories before files, each group alphabetical', () => {
    ...
    // "alpha.txt" before "README.md": alphabetical, not the byte order that would hoist every capitalized name to the front
  ...
  /** Git stores trees as if their names ended in "/", so "src.d" sorts ahead of "src" in the raw listing. Sorting by name puts that right. */
