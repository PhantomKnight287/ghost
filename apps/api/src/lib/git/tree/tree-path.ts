...
/** NUL survives percent-decoding and would terminate the argument git actually sees; the other control characters have no business in a URL query. */
...
/** The file form of {@link normalizeTreePath}: same rejections, no trailing slash, and the root is not a file so an empty path is refused. */
