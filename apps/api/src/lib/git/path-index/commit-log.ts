...
/** Walks history oldest-first with the paths each commit touched. Merges stay opaque, so a file is attributed to the commit that wrote it, and `--reverse --topo-order` means the last write per path is the newest. */
...
function parseRecord(record: string): CommitRecord | null {
  // A subject can hold anything but NUL, so the first NUL is the only boundary that is safe to split the header on.
