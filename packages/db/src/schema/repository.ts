...
import { pgEnum, pgTable, primaryKey, text, timestamp, uniqueIndex } from "drizzle-orm/pg-core";
...
export const repository = pgTable("repository", {
  ...
  // points to the default branch of the repository. will be set to the first branch that shows up in push
  defaultBranch:text(),
 ...
 */
export const repositoryPathCommit = pgTable("repository_path_commit", {
  repositoryId: text().references(() => repository.id, { onDelete: "cascade" }).notNull(),
  ref: text().notNull(),
  ...
  primaryKey({ columns: [t.repositoryId, t.ref, t.path] }),
])
 ...
 */
export const repositoryRefIndex = pgTable("repository_ref_index", {
  repositoryId: text().references(() => repository.id, { onDelete: "cascade" }).notNull(),
  ref: text().notNull(),
  indexedCommitSha: text().notNull(),
  updatedAt: timestamp().notNull().defaultNow().$onUpdateFn(() => new Date()),
}, (t) => [
  primaryKey({ columns: [t.repositoryId, t.ref] }),
])
